  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        <motion.button whileTap={{ scale: 0.95 }} onClick={() => { setEditing(null); setForm(EMPTY); setShowPassword(false); setModal(true); }} className="btn-primary flex items-center gap-2 self-start">
          ...
          <table className="w-full text-sm">
            ...
            <tbody className="divide-y divide-slate-100 dark:divide-slate-700">
              {loading ? <tr><td colSpan={6} className="text-center py-8 text-slate-400">Loading...</td></tr>
                : filtered.length === 0 ? <tr><td colSpan={6} className="text-center py-8 text-slate-400">No faculty found</td></tr>
                  : filtered.map((f, i) => (
            ...
            <motion.div initial={{ scale: 0.95 }} animate={{ scale: 1 }} exit={{ scale: 0.95 }} className="bg-white dark:bg-slate-800 rounded-2xl shadow-2xl w-full max-w-2xl max-h-[90vh] overflow-y-auto">
              ...
              <form onSubmit={handleSubmit} className="p-6 grid grid-cols-1 sm:grid-cols-2 gap-4">
                {[
                  ...
                  { label: "Subjects (comma sep)", name: "subjects", type: "text" },
                ].map(f => (
