...
export default function StudentAttendance() {
    ...
    const getLocation = () => {
        if (!navigator.geolocation) {
            setLocationError("GPS supported nahi hai aapke device mein!");
            return;
        }
...
const submitAttendance = async () => {
    if (!selfie) return setMessage("Pehle selfie lo!");
    if (!location) return setMessage("Location allow karo pehle!");
    if (!subject) return setMessage("Subject select karo!");
...
return (
    <div className="p-6 max-w-4xl mx-auto">
        <h1 className="text-2xl font-bold text-gray-800 dark:text-white mb-6">📍 Attendance Mark Karo</h1>
        ...
        <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-lg p-6 mb-6">
            ...
            >
                {loading ? "Submit ho raha hai..." : "✅ Attendance Submit Karo"}
            </button>
        </div>

        {/* Records */}
        <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-lg p-6">
            <h2 className="text-lg font-semibold mb-4 text-gray-700 dark:text-gray-200">Meri Attendance History</h2>
            {myRecords.length === 0 ? (
                <p className="text-gray-400 text-center py-4">Koi record nahi abhi tak</p>
            ) : (
