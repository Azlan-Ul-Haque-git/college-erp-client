          ...
          return (
            <NavLink key={item.path} to={item.path}
              ...
              className={`flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-medium transition-all duration-150 group ${isActive
                ...
                : "text-slate-600 dark:text-slate-400 hover:bg-slate-100 dark:hover:bg-slate-700/60 hover:text-slate-800 dark:hover:text-white"
                }`}>
              <item.icon className={`w-4 h-4 flex-shrink-0 transition-transform duration-150 ${isActive ? "scale-110" : "group-hover:scale-105"}`} />
      ...
      <div className="flex-shrink-0 px-3 py-3 border-t border-slate-100 dark:border-slate-700/60">
        <div className="flex items-center justify-between">
          ...
          <button onClick={() => { logout(); navigate("/login"); }}
            className="flex items-center gap-1.5 text-xs text-red-500 hover:text-white hover:bg-red-500 font-semibold px-3 py-2 rounded-xl transition-all duration-150">
    ...
    <>
      ...
      <div className="lg:hidden fixed top-0 left-0 right-0 z-40 h-16
        ...
        <div className="flex items-center gap-2">
          <div className={`w-7 h-7 rounded-xl bg-gradient-to-br ${rc.color} flex items-center justify-center`}>
        ...
        {/* Avatar */}
        <div className="w-9 h-9 rounded-xl overflow-hidden ring-2 ring-offset-1"
          style={{ ringColor: rc.bg }}>
          {user?.avatar ? (
            <img src={user.avatar} alt="avatar" className="w-full h-full object-cover" />
          ) : (
            <div className={`w-full h-full bg-gradient-to-br ${rc.color} flex items-center justify-center text-white font-bold text-sm`}>
              {user?.name?.[0]?.toUpperCase()}
            </div>
          )}
        </div>
